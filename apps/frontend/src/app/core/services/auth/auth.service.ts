...
import { UserRole, IUserResponse as User, USER_ROLES, IAuthResponse, getInitialLetter } from '@eduno/shared';
...
export class AuthService {
  ...
  checkSession(): Observable<User | null> {
    return this.http
      .get<IAuthResponse>('/auth/get-session')
      .pipe(
        map((res) => {
  ...
  login(email: string, password: string): Observable<User> {
    return this.http
      .post<IAuthResponse>('/auth/sign-in/email', { email, password })
      .pipe(
  ...
  register(name: string, email: string, password: string): Observable<User> {
    return this.http
      .post<IAuthResponse>('/auth/sign-up/email', { name, email, password })
      .pipe(
        map((res) => ({ ...res.user, initialLetter: getInitialLetter(res.user.name) })),
        tap((user) => {
          this._currentUser.set(user);
  ...
  listAccounts(): Observable<{ provider: string, id: string, accountId: string }[]> {
    return this.http.get<{ provider: string, id: string, accountId: string }[]>('/auth/list-accounts');
  }
